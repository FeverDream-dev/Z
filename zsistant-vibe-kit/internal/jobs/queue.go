...
// Queue manages durable jobs for an agent.
type Queue struct {
	workspace string
}
...
func NewQueue(workspace string) *Queue {
	return &Queue{workspace: workspace}
}
...
func (q *Queue) Update(job *Job) error {
	return q.Enqueue(job) // overwrite
}

// List returns all jobs in the queue.
func (q *Queue) List() ([]*Job, error) {
	dir := filepath.Join(q.workspace, "jobs")
...
// jobPath returns the filesystem path for a job.
func (q *Queue) jobPath(jobID string) string {
	return filepath.Join(q.workspace, "jobs", jobID)
}
...
type LoopDetector struct {
	threshold int // max failures before considering it a loop
}
